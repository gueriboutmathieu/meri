...
def test_query_llm_command_statement(mocker: MockerFixture):
    ...
    llm_response = query_command(context, "I am a statement.")
...
def test_query_llm_command_question(mocker: MockerFixture):
    ...
    statement = Statement(id=uuid7(), content="A question is the opposite of a statement.", vector=[0.0])  # pyright: ignore
    mocker.patch.object(context.embedding_service, "embed", return_value=[0.0])
    ...
    llm_response = query_command(context, "What is a question ?")
